<?php
 include("../../../includes/header.inc"); ?>
<H3 align="center">Corso Linux di Base</H3>
<p>
Si tiene presso la Facolt&agrave; di Ingegneria dell'Universit&agrave;
di Brescia una serie di lezioni di base su Linux nell'ambito del corso <b>
Sistemi Operativi A</b>. La frequenza alle lezioni &egrave; aperta al
pubblico.
<H4 CLASS="programma">Informazioni generali</H4>

Il corso introduce Linux dal punto di vista di:
<OL>
 <LI> comandi di base;
 <LI> programmazione di shell;
 <LI> cenni sull'amministrazone del sistema.
</OL>
Non &egrave; correntemente prevista la trattazione dell'ambiente
 grafico, X e le applicazioni con esso connesse (Gnome, KDE, etc.).
<p>
Le lezioni si terranno ogni Venerd&igrave; sino a poco
 prima di Natale (termine del quadrimestre), dalle ore 16:30 alle ore
 18:00, presso l'aula <i>MTB</i> della facolt&agrave; di Ingegneria.
<DIV CLASS="programma">
<H4 CLASS="programma">Programma dettagliato</H4>
<DL>
 <DT> Venerd&igrave; 11 Ottobre:
  <DD><OL>
  <LI> il software free;
  <LI> cenni storici;
  <LI> caratteristiche di Linux;
  <LI> installazione di Linux.
  </OL>
 <DT> Venerd&igrave; 18 Ottobre:
  <DD><OL>
  <LI> login e sessione utente;
  <LI> comandi di base;
  <LI> organizzazione del filesystem.
  </OL>
 <DT> Venerd&igrave; 8 Novembre:
  <DD><OL>
  <LI> Tema: filesystem Linux
  <LI> Dettagli: organizzazione delle directory, tipi di file, permessi, link
  </OL>
 <DT> Venerd&igrave; 15 Novembre:
  <DD><OL>
  <LI> Tema: programmazione di script
  <LI> Dettagli: struttura di uno script, variabili, test, condizioni, cicli
  </OL>
</DL>
</DIV>

<?php
include("../../../includes/footer.inc");
?>
